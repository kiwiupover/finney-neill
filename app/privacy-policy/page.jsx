...
export async function getProps() {
  const policyTableId = 'tblKURV5bSHXLRBOn'
...
export async function generateMetadata() {
  const { policyContent } = await getProps()

  const image =
    'https://res.cloudinary.com/dhqv889nh/image/upload/v1682869348/baby-hands_xd5aqh.jpg'

  const title = policyContent.seo_title
  const description = policyContent.seo_description
  return {
    title,
    description,
    alternates: {
...
export default async function TermsAndServicesPage() {
  ...
  return (
    <div className="relative isolate">
      ...
      <TermsServicesPolicy content={policyContent} />
    </div>
  )
}
