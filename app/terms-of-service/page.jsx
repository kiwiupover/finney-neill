...
import { TermsServicesPolicy } from '@/components/TermsServicesPolicy'
...
export async function getProps() {
  const termsServicesTableId = 'tblak1tcWADwVT0d8'

  const response = await contentPage({
    tableId: termsServicesTableId,
  ...
  return {
    termsServicesContent: response,
  }
}

export async function generateMetadata() {
  const { termsServicesContent } = await getProps()

  const image =
    'https://res.cloudinary.com/dhqv889nh/image/upload/v1682869348/baby-hands_xd5aqh.jpg'

  const title = termsServicesContent.seo_title
  const description = termsServicesContent.seo_description
  return {
    title,
    description,
    alternates: {
...
export default async function TermsAndServicesPage() {
  ...
  return (
    <div className="relative isolate">
      ...
      <TermsServicesPolicy content={termsServicesContent} />
    </div>
  )
}
